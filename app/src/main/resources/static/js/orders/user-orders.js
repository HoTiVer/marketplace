...
async function loadOrders(pageNumber = 0) {
    ...
    try {
        const res = await fetchWithAuth(`/api/v1/order/orders?page=${pageNumber}&size=${size}`);
        if (!res.ok) {
...
function createOrderCard(o) {
    ...
    cancelBtn.addEventListener('click', async () => {
        ...
        try {
            const res = await fetchWithAuth(`/api/v1/order/orders/${o.orderId}/cancel`, { method: 'PATCH' });
            if (res.ok) {
