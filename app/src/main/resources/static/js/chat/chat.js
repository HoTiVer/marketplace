...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function loadChat() {
        try {
            ...
            chatTitle.textContent = `Chat with ${chat.participantIds.filter(id => id !== userData.id).join(", ")}`;

            // Render messages
            messagesEl.innerHTML = "";
            chat.messages.forEach(msg => {
                const msgDiv = document.createElement("div");
                msgDiv.className = msg.senderId === userData.id
                    ? "self-end bg-blue-100 text-blue-900 px-4 py-2 rounded-xl max-w-[70%] self-end"
                    : "bg-gray-200 text-gray-900 px-4 py-2 rounded-xl max-w-[70%] self-start";
                msgDiv.innerHTML = `<p class="font-medium">${msg.senderName}</p><p>${msg.content}</p>
